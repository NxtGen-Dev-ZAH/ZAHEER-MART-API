...
from app import order_pb2, kafka, settings, auth, models
import logging
...
async def login_user(
    login: Annotated[OAuth2PasswordRequestForm, Depends(OAuth2PasswordRequestForm)]
):
    user_proto = order_pb2.User(
        username=login.username,
        service=order_pb2.SelectService.ORDER,
        password=login.password,
        option=order_pb2.SelectOption.LOGIN,
    )
    ...
    )# USER TOPIC KAFKA AND THIS WILL BE SAME ...
    ...
    if user_proto.error_message or user_proto.http_status_code:
        raise HTTPException(
            status_code=user_proto.http_status_code, detail=user_proto.error_message
        )
    return {
        "access_token": user_proto.access_token,
        "token_type": "bearer",
        "refresh_token": user_proto.refresh_token,
    }
...
async def get_current_user(
    verify_token: Annotated[str, Depends(auth.verify_access_token)]
):
    ...
    logger.info(
        f"Username that we get after decoding token and ready to send to user_service to get user detail:{verify_token}"
    )
    user_proto = order_pb2.User(
    ...
    logger.info(
        f"User detail that we get from user service at user/me endpoint :{verify_token}"
    )
    return {
        "id": user_proto.id,
