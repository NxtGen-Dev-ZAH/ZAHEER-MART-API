...
from app import settings, product_pb2,db,models
...
async def handle_get_all_products():
    with Session(db.engine) as session:
        products_list = session.exec(select(models.Producttable)).all()
...
async def handle_get_product(product_id):
    with Session(db.engine) as session:
        product = session.exec(select(models.Producttable).where(models.Producttable.product_id == product_id)).first()
        if product:
            product_proto = product_pb2.Product(
                id=product.id,
                product_id=str(product.product_id),
...
async def handle_update_product(new_msg):
    with Session(db.engine) as session:
        product = session.exec(select(models.ProductUpdate).where(models.ProductUpdate.product_id == new_msg.product_id)).first()
        if product:
...
async def consume_message_for_stock_level_update():
    ...
    try:
        async for msg in consumer:
            ...
            logger.info(f"Received message: {new_msg}")
            with Session(db.engine) as session:
                product = session.exec(select(models.Producttable).where(models.Producttable.product_id == new_msg.product_id)).first()
